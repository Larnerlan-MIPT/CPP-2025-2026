...
public:
    ...
    void handleEvents()
    {
        ...
        if (GetMouseDelta().x != 0 || GetMouseDelta().y != 0) onMouseMoved(mouse);
    }
...
public:
    DraggableWithColorChange(Vector2 pos, Vector2 size, Color base, Color drag)
        : Draggable(pos, size, base), baseColor(base), dragColor(drag) {}
    void handleEvents()
    {
...
int main()
{
    InitWindow(800, 600, "Draggable with Color Change");
    SetTargetFPS(60);

    DraggableWithColorChange d {{100,100}, {200,150}, RED, GREEN};
